        ...
        <transition appear name="slideFadeIn">
            <div class="search-box-wrapper" v-show="searchBoxFocue">
                <div class="hot-words" v-show="searchValue === ''">
                    <h3>热搜排行</h3>
                    <ul class="words-list">
                        <li v-for="(item,index) in hotWords" :key="index" :class="highlightColor(index)" @click="quickSelect(item.k)">
                ...
                <div class="loading-div" v-show="loadingState" v-loading="{loading:loadingState,color:'red',title:'努力搜索中...'}"></div>                
  ...
  methods: {
    ...
    async getSearchResult() {
      try {
        ...
        let res = await Api.HomeApi.getHotWordsSearch(this.searchValue);
        this.loadingState = false;
        this.searchResult = res.data.list.splice(0, 15);
      } catch (error) {
        this.loadingState = false;
        console.log(error);
      }
    ...
    async getHotWords() {
      let res = await Api.HomeApi.getHotWords();
      this.hotWords = res.data.splice(0, 17);
    },
    focus() {
